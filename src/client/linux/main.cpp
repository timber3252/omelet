...
char server_ip[kMinBufSize] = "127.0.0.1",
     client_listen_ip[kMinBufSize] = "0.0.0.0";
uint16_t server_port = 21121, client_listen_port = 36868;
uint32_t client_sock, client_listen_sock, peer_socks[kMaxConnections],
    self_virtual_ip;
ConsoleLog logc;
int tun_fd, nread;
...
void init() {
  ...
  if (bind(client_listen_sock, (sockaddr *)&client_listen_addr,
           sizeof client_listen_addr) == -1) {
    ...
    if (nrecv <= 0) {
      ...
      switch (result[0]) {
        ...
        case PACKET_FIRST_CONFIRM: {
          ...
          logc(LogLevel::Info)
              << "Allocated IP: " << int(self_ip.ipv4_address_s[0]) << '.'
              << int(self_ip.ipv4_address_s[1]) << '.'
              << int(self_ip.ipv4_address_s[2]) << '.'
              << int(self_ip.ipv4_address_s[3]) << "(" << self_ip.ipv4_address_i
              << ")";
          int ret = set_host_addr(tun_name, self_ip.ipv4_address_i);
          if (ret < 0) {
            logc(LogLevel::Fatal)
                << "Failed to set address of tun device. (errcode = " << ret
                << ")";
            exit(-1);
          }
          continue;
        ...
        case PACKET_QUERY_CLIENT_LIST: {
          logc(LogLevel::Debug)
              << "Packet decrypted: query client list response";
          ...
          do {
            ...
            port = result[cnt++];
            port |= (result[cnt++] << 8);
            logc(LogLevel::Info) << virtual_ip.ipv4_address_i << ' '
                                 << nat_ip.ipv4_address_i << ' ' << port;
            router.insert(virtual_ip.ipv4_address_i, nat_ip.ipv4_address_i,
                          port);
          } while (--ncnt > 0);
          continue;
        }
...
void *wait_thread(void *) {
  logc(LogLevel::Info) << "Listening on " << client_listen_ip << ':'
                       << client_listen_port;
  while (true) {
    sockaddr_in peer{};
...
void *async_wait_and_forward(void *p) {
  ...
  for (int i = 0; i < 22; ++i) {
    ...
    if (res == nullptr) {
      ...
      if (res->sockfd == 0 ||
          send(res->sockfd, data->encrypted_data, data->length, 0) < 0) {
        ...
        if (connect(sockfd, (sockaddr *)&addr, sizeof(addr)) == -1) {
          logc(LogLevel::Error) << "Failed to forward message to the peer ("
                                << res->ip << ':' << res->port << ").";
          continue;
        }
        if (res->sockfd) close(res->sockfd);
...
int main(int argc, char *argv[]) {
  ...
  uint8_t buffer[kProtocolMaxSize], req_query_client_list[kMinBufSize],
      output[kMinBufSize];
  tun_name[0] = '\0';
  ...
  while (true) {
    ...
    logc(LogLevel::Debug) << (int)dest_ip.ipv4_address_s[3] << '.'
                          << (int)dest_ip.ipv4_address_s[2] << '.'
                          << (int)dest_ip.ipv4_address_s[1] << '.'
                          << (int)dest_ip.ipv4_address_s[0] << ' '
                          << dest_ip.ipv4_address_i;
    ...
    if (res == nullptr) {
      ...
      data->length =
          (size_t)ceil((nread + 2) / (double)AES_BLOCK_SIZE) * AES_BLOCK_SIZE;
      ...
      if (res->sockfd == 0 ||
          send(res->sockfd, encrypted_data,
               (size_t)ceil((nread + 2) / (double)AES_BLOCK_SIZE) *
                   AES_BLOCK_SIZE,
               0) < 0) {
        int sockfd = socket(PF_INET, SOCK_STREAM, 0);
        if (sockfd < 0) {
